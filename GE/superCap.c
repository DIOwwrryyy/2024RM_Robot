...
void CapSendInit(uint8_t powerBuffer , uint8_t nowPower , uint16_t volt)
{
	...
	if(KeyBoard_c&&KeyBoard_ctrl){
	OUT_switch=1;
	}//judge_user_data.robot_status.chassis_power_limit=60;
	capData_t.capSetData.dataNeaten.maxPower =60;//judge_user_data.robot_status.chassis_power_limit;//judge_user_data.robot_status.chassis_power_limit;		//最大功率
	...
	capData_t.capSetData.dataNeaten.out_switch =1;//OUT_switch;////C键作为电容强制开关，上锁模式。默认离线，按一下打开，
	//超级电容发送函数
	my_CAN_Send(&hcan2 , 0x1ff , capData_t.capSetData.sendData[0] , capData_t.capSetData.sendData[1] , \
													capData_t.capSetData.sendData[2] , capData_t.capSetData.sendData[1] , 8);
}
...
void CanManage_cap(uint8_t* can_data , struct capGetData_typdef* data)
{
	...
	capData_t.capGetDate.cap_realy_out=capData_t.capGetDate.nowPower-judge_user_data.robot_status.chassis_power_limit;;
...
void SuperCapTransation(uint8_t mod , int32_t setOut , uint64_t time)
{
	...
	const uint64_t outTime_1 = 60;  //超级电容进入时的斜坡长度//单位毫秒
	const uint64_t outTime_2= 100;  //超级电容退出时的斜坡长度//单位毫秒
	const uint16_t capValt = 8;   //制退出的电压阈值
...
//*	float raid_3=0.1;
	...
	uint16_t slope_err = SuperMaxPower - judge_user_data.robot_status.chassis_power_limit;		//斜坡功率总值
//	mod=remote_t.key_board.ctrl;
	...
	{  //超级电容模式开启   
		...
		if (capData_t.capGetDate.capVolt>21)
			Inter=raid_1*((capData_t.capGetDate.capVolt)*5.8f+judge_user_data.robot_status.chassis_power_limit);//judge_user_data.robot_status.chassis_power_limit
		if (capData_t.capGetDate.capVolt<=21&&capData_t.capGetDate.capVolt>18)
			Inter=raid_1*((capData_t.capGetDate.capVolt)*4.9f+judge_user_data.robot_status.chassis_power_limit);
		if (capData_t.capGetDate.capVolt<=18&&capData_t.capGetDate.capVolt>=12)
			Inter=raid_1*((capData_t.capGetDate.capVolt)*4.3+judge_user_data.robot_status.chassis_power_limit);
		if (capData_t.capGetDate.capVolt>=9&&capData_t.capGetDate.capVolt<12)
			Inter=raid_1*((capData_t.capGetDate.capVolt)*3.7f+judge_user_data.robot_status.chassis_power_limit);
		if (capData_t.capGetDate.capVolt<9)
			Inter=raid_1*((capData_t.capGetDate.capVolt)*1.4f+judge_user_data.robot_status.chassis_power_limit);
		PowerLimBuffer_Cap(judge_user_data.robot_status.chassis_power_limit+Inter , capData_t.capGetDate.buffer , setOut);
	...
	{  //超级电容模式退出
		...
		PowerLimBuffer_Cap(judge_user_data.robot_status.chassis_power_limit , judge_user_data.power_heat_data.buffer_energy , setOut);
...
//	uint16_t slope_err = SuperMaxPower - judge_user_data.power.maxPower;		//斜坡功率总值
...
//		PowerLimBuffer_Cap(judge_user_data.power.maxPower+slope_out , capData_t.capGetDate.buffer , setOut);
//	}
//}
