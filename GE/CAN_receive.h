 ...
 struct canBoll_typedef
{
		...
		}__attribute__ ((__packed__)) my_can1;
		 struct 
		{
			uint8_t RXboll :1;
			uint8_t RXMailBox :3;
			...
			uint8_t TxState :3;
		}__attribute__ ((__packed__)) my_can2;
};
...
extern uint8_t can_send_data[8];   //can发送数据暂存

extern uint8_t can_union_txdata[8];//can上下板发送共用体数据暂存
